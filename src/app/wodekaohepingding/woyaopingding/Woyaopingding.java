package  app.wodekaohepingding.woyaopingding;
...
public class Woyaopingding extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/wodekaohepingding/woyaopingding/woyaopingding.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String kaopingmingchenSearch = request.getParameter("kaopingmingchenSearch"); 
 if( kaopingmingchenSearch != null && !kaopingmingchenSearch.equals("") ){  
 	 where.put("kaopingmingchenSearch", kaopingmingchenSearch);  
 } 
String kaopingduixiangSearch = request.getParameter("kaopingduixiangSearch"); 
 if( kaopingduixiangSearch != null && !kaopingduixiangSearch.equals("") ){  
 	 where.put("kaopingduixiangSearch", kaopingduixiangSearch);  
 } 
String tijiaorenSearch = request.getParameter("tijiaorenSearch"); 
 if( tijiaorenSearch != null && !tijiaorenSearch.equals("") ){  
 	 where.put("tijiaorenSearch", tijiaorenSearch);  
 } 
String tijiaoshijianSearch = request.getParameter("tijiaoshijianSearch"); 
 if( tijiaoshijianSearch != null && !tijiaoshijianSearch.equals("") ){  
 	 where.put("tijiaoshijianSearch", tijiaoshijianSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Woyaopingding.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String kaopingmingchen = request.getParameter("kaopingmingchenHidd"); 
where.put("kaopingmingchen", kaopingmingchen);  
String kaopingduixiang = request.getParameter("kaopingduixiangHidd"); 
where.put("kaopingduixiang", kaopingduixiang);  
String tijiaoren = request.getParameter("tijiaoren"); 
where.put("tijiaoren", tijiaoren);  
String tijiaoshijian = request.getParameter("tijiaoshijian"); 
where.put("tijiaoshijian", tijiaoshijian);  
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库新增数据
            sqlMap.insert("Woyaopingding.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
            ...
            //新增数据
		    sqlMap.insert("Woyaopingding.insertObj", whereMap);
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库提交修改数据
            sqlMap.update("Woyaopingding.updateObj", where);
    ...
    public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//向数据库提交修改信息
		    sqlMap.update("Woyaopingding.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Woyaopingding.doDeleOrUnDele", where);
            
             where.put("tableName", "pingdingmingxi" );
 sqlMap.delete("Woyaopingding.doDeleErJiData", where); 
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Woyaopingding.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Woyaopingding.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "考评名称", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "考评对象", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "提交人", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "提交时间", wcfFC5);  
 ws.addCell(label_3); 
 Label label_4 = new Label(5, 3, "状态", wcfFC5);  
 ws.addCell(label_4); 
 ...
 ws.addCell(label_7); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String kaopingmingchen = ((HashMap) (al.get(i))).get("kaopingmingchen").toString();  
 label_0 = new Label(1, i + 4, kaopingmingchen, wcfF4); 
 ws.addCell(label_0); 
 String kaopingduixiang = ((HashMap) (al.get(i))).get("kaopingduixiang").toString();  
 label_1 = new Label(2, i + 4, kaopingduixiang, wcfF4); 
 ws.addCell(label_1); 
 String tijiaoren = ((HashMap) (al.get(i))).get("tijiaoren").toString();  
 label_2 = new Label(3, i + 4, tijiaoren, wcfF4); 
 ws.addCell(label_2); 
 String tijiaoshijian = ((HashMap) (al.get(i))).get("tijiaoshijian").toString();  
 label_3 = new Label(4, i + 4, tijiaoshijian, wcfF4); 
 ws.addCell(label_3); 
 String shenpi = ((HashMap) (al.get(i))).get("shenpi").toString();  
 label_4 = new Label(5, i + 4, shenpi, wcfF4); 
 ws.addCell(label_4); 
 ...
 ws.addCell(label_7); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Woyaopingding.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Woyaopingding.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Woyaopingding.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Woyaopingding.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Woyaopingding.doDelePic", where);
