package  app.kaohepingdingguanli.kaopingxiangmushezhi;
...
public class Kaopingxiangmushezhi extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/kaohepingdingguanli/kaopingxiangmushezhi/kaopingxiangmushezhi.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String kaopingxiangmingchenSearch = request.getParameter("kaopingxiangmingchenSearch"); 
 if( kaopingxiangmingchenSearch != null && !kaopingxiangmingchenSearch.equals("") ){  
 	 where.put("kaopingxiangmingchenSearch", kaopingxiangmingchenSearch);  
 } 
String kaopingbiaozhunSearch = request.getParameter("kaopingbiaozhunSearch"); 
 if( kaopingbiaozhunSearch != null && !kaopingbiaozhunSearch.equals("") ){  
 	 where.put("kaopingbiaozhunSearch", kaopingbiaozhunSearch);  
 } 
String kaopingshuomingSearch = request.getParameter("kaopingshuomingSearch"); 
 if( kaopingshuomingSearch != null && !kaopingshuomingSearch.equals("") ){  
 	 where.put("kaopingshuomingSearch", kaopingshuomingSearch);  
 } 
String zhidingrenSearch = request.getParameter("zhidingrenSearch"); 
 if( zhidingrenSearch != null && !zhidingrenSearch.equals("") ){  
 	 where.put("zhidingrenSearch", zhidingrenSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Kaopingxiangmushezhi.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Kaopingxiangmushezhi.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("kaopingxiangmingchen", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("kaopingbiaozhun", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("kaopingshuoming", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("zhidingren", item.getString("gb2312"));
}
else if (i == 4) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Kaopingxiangmushezhi.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Kaopingxiangmushezhi.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Kaopingxiangmushezhi.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Kaopingxiangmushezhi.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Kaopingxiangmushezhi.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Kaopingxiangmushezhi.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "考评项名称", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "考评标准", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "考评说明", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "制定人", wcfFC5);  
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String kaopingxiangmingchen = ((HashMap) (al.get(i))).get("kaopingxiangmingchen").toString();  
 label_0 = new Label(1, i + 4, kaopingxiangmingchen, wcfF4); 
 ws.addCell(label_0); 
 String kaopingbiaozhun = ((HashMap) (al.get(i))).get("kaopingbiaozhun").toString();  
 label_1 = new Label(2, i + 4, kaopingbiaozhun, wcfF4); 
 ws.addCell(label_1); 
 String kaopingshuoming = ((HashMap) (al.get(i))).get("kaopingshuoming").toString();  
 label_2 = new Label(3, i + 4, kaopingshuoming, wcfF4); 
 ws.addCell(label_2); 
 String zhidingren = ((HashMap) (al.get(i))).get("zhidingren").toString();  
 label_3 = new Label(4, i + 4, zhidingren, wcfF4); 
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Kaopingxiangmushezhi.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Kaopingxiangmushezhi.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Kaopingxiangmushezhi.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Kaopingxiangmushezhi.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Kaopingxiangmushezhi.doDelePic", where);
