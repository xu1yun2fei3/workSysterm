		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					xtype : 'textfield',
					fieldLabel : '调查人群',
					name : 'diaocharenqunSearch',
					id : 'diaocharenqunSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '调查内容',
					name : 'diaochanarongSearch',
					id : 'diaochanarongSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'diaochamingchen', 'diaocharenqun', 'diaochanarong', 'fabushijian', 'faburen', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd', 'fuJian' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/xitongdiaochaguanli/fabuxitongdiaocha.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		 ...
		 */
		{
			...
			header : '<font color="blue">调查人群</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'diaocharenqun'
		}, {
			header : '<font color="blue">调查内容</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'diaochanarong'
		}, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.diaocharenqunSearch = document.all.diaocharenqunSearch.value;
	ManageObj.dataStore.baseParams.diaochanarongSearch = document.all.diaochanarongSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;调查人群',
					name : 'diaocharenqun',
					id : 'diaocharenqun',
					anchor : '95%',
					allowBlank : false
				...
				items : [ {
					xtype : 'textarea',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;调查内容',
					name : 'diaochanarong',
					id : 'diaochanarong',
					height : 400,
					anchor : '95%',
	...
	if (rowIndex != null) {
		...
		ManageObj.operationFormPan.findById('diaocharenqun').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('diaocharenqun');
		ManageObj.operationFormPan.findById('diaochanarong').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('diaochanarong');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xitongdiaochaguanli/fabuxitongdiaocha.do?flag=doExportExcel";
	window.open(url);
};
