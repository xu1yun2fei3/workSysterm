package  app.xitonggaojishezhi.pingdingfenshu;
...
public class Pingdingfenshu extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/xitonggaojishezhi/pingdingfenshu/pingdingfenshu.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String pingdingfenshuSearch = request.getParameter("pingdingfenshuSearch"); 
 if( pingdingfenshuSearch != null && !pingdingfenshuSearch.equals("") ){  
 	 where.put("pingdingfenshuSearch", pingdingfenshuSearch);  
 } 
...
String attr5Search = request.getParameter("attr5Search"); 
 if( attr5Search != null && !attr5Search.equals("") ){  
 	 where.put("attr5Search", attr5Search);  
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingfenshu.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String pingdingfenshu = request.getParameter("pingdingfenshu"); 
where.put("pingdingfenshu", pingdingfenshu);  
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库新增数据
            sqlMap.insert("Pingdingfenshu.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
            ...
            //新增数据
		    sqlMap.insert("Pingdingfenshu.insertObj", whereMap);
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库提交修改数据
            sqlMap.update("Pingdingfenshu.updateObj", where);
    ...
    public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//向数据库提交修改信息
		    sqlMap.update("Pingdingfenshu.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Pingdingfenshu.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Pingdingfenshu.doShenPi", where);
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Pingdingfenshu.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "评定分数", wcfFC5);  
 ws.addCell(label_0); 
 ...
 ws.addCell(label_3); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String pingdingfenshu = ((HashMap) (al.get(i))).get("pingdingfenshu").toString();  
 label_0 = new Label(1, i + 4, pingdingfenshu, wcfF4); 
 ws.addCell(label_0); 
 ...
 ws.addCell(label_3); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Pingdingfenshu.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Pingdingfenshu.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Pingdingfenshu.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingfenshu.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Pingdingfenshu.doDelePic", where);
