...
ManageObj.doUpdateSubmit = function(rowIndex) {
	...
	/* form表单提交 */
	ManageObj.operationFormPan
			.getForm()
			.submit(
					{
						waitMsg : '数据提交中......',
						method : 'post',
						url : '/WEB-ROOT/app/sysManage/systemRoleManage.do?flag=doUpdateSubmit'
								+ '&id='
								+ ManageObj.dataGridPan.getStore().getAt(
										rowIndex).get('id')
						...
						failure : function(form, action) {
							Ext.MessageBox.alert(WARRING_WIN_TITLE,
									action.result.msg);
						}
					});
};
...
ManageObj.update4ej = function(url, flag) {
	...
	ManageObj.updateWindow = new Ext.Window({
		...
		resizable : false,
		layout : 'fit',
		html : "<iframe scrolling=no src='" + url
