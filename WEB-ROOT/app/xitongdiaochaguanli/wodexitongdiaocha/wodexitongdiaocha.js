var ManageObj = function() {
	...
	ManageObj.diaochamingchenGuanLianStore = new Ext.data.JsonStore({
		url : '/WEB-ROOT/app/xitongdiaochaguanli/wodexitongdiaocha.do',
		baseParams : {
			flag : 'doGuanLian',
			guanLianBiao : 'fabuxitongdiaocha',
			guanLianZiDuan : 'diaochamingchen'
		},
		root : 'root',
		totalProperty : 'totalProperty',
		fields : [ 'diaochamingchen' ]
	});
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					xtype : 'textfield',
					fieldLabel : '调查名称',
					name : 'diaochamingchenSearch',
					id : 'diaochamingchenSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '调查结果',
					name : 'diaochajieguoSearch',
					id : 'diaochajieguoSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		totalProperty : 'totalProperty',
		remoteSort : true,
		fields : [ 'diaochamingchen', 'diaochajieguo', 'tijiaoshijian', 'tijiaoren', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd' ],
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		 ...
		 */
		{
			...
			header : '<font color="blue">调查名称</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'diaochamingchen'
		}, {
			header : '<font color="blue">调查结果</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'diaochajieguo'
		}, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.diaochamingchenSearch = document.all.diaochamingchenSearch.value;
	ManageObj.dataStore.baseParams.diaochajieguoSearch = document.all.diaochajieguoSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'combo',
					fieldLabel : '<font color="red">*</font>&nbsp;调查名称',
					store : ManageObj.diaochamingchenGuanLianStore,
					valueField : 'diaochamingchen',
					displayField : 'diaochamingchen',
					mode : 'remote',
					forceSelection : false,
					hiddenName : 'diaochamingchenHidd',
					triggerAction : 'all',
					editable : true,
					name : 'diaochamingchen',
					id : 'diaochamingchen',
					anchor : '95%',
					allowBlank : false,
					listeners : {
						change : function loadCallBack(store, records, options) {
							getDiaoChaNeiRong(store, records, options);
						}
					}
				} ]
			}, {
				...
				items : [ {
					xtype : 'textarea',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;调查结果',
					name : 'diaochajieguo',
					id : 'diaochajieguo',
					height:400,
					anchor : '95%',
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('diaochamingchen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('diaochamingchen');
		ManageObj.operationFormPan.findById('diaochajieguo').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('diaochajieguo');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xitongdiaochaguanli/wodexitongdiaocha.do?flag=doExportExcel";
	window.open(url);
};
...
function getDiaoChaNeiRong(){
	var diaochamingchen = document.all.diaochamingchen.value ;
	Ext.Ajax.request({
		waitMsg : '数据提交中......',
		url : '/WEB-ROOT/app/xitongdiaochaguanli/wodexitongdiaocha.do',
		method : 'POST',
		success : function(response, options) {
			Ext.getCmp('diaochajieguo').setValue( response.responseText ) ;
		},
		failure : function(response, options) {
			Ext.MessageBox.alert('温馨提醒！', responseArray.msg);
		},
		params : {
			flag : "diaochaneirong",
			diaochamingchen : diaochamingchen
		}
	});
}
