		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					xtype : 'textfield',
					fieldLabel : '消息标题',
					name : 'xiaoxibiaotiSearch',
					id : 'xiaoxibiaotiSearch',
					anchor : '100%'
				} ]
			}, {
				columnWidth : .2,
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '阅读状态',
					name : 'yueduzhuangtaiSearch',
					id : 'yueduzhuangtaiSearch',
					anchor : '100%'
				} ]
			} ]
		} ],
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'xiaoxibiaoti', 'xiaoxinarong', 'jieshouren', 'faburen', 'fabushijian', 'yueduzhuangtai', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd', 'fuJian' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/xiaoxiguanli/zhannaxiaoxi.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		 ...
		 */
		{
			...
			header : '<font color="blue">消息标题</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'xiaoxibiaoti'
		}, {
			...
			header : '<font color="blue">接收人</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'jieshouren'
		}, {
			...
			header : '<font color="blue">阅读状态</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'yueduzhuangtai'
		}, {
...
ManageObj.operatorRenderer = function(id, cellmeta, record, rowIndex, columnIndex, store) {
	var returnStr = "<a href=\"#\" onclick=\"ManageObj.update(" + rowIndex + ",'view','"+record.data['jieshouren']+"');\">浏览</a>";
	
	if( LoginUserAcct == record.data['jieshouren'] ){
		returnStr += "&nbsp;|&nbsp;<a href=\"#\" onclick=\"ManageObj.update(" + rowIndex + ",'back','"+record.data['jieshouren']+"');\">回复</a>";
	}else{
		if (R == "n") {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.xiaoxibiaotiSearch = document.all.xiaoxibiaotiSearch.value;
	ManageObj.dataStore.baseParams.xiaoxinarongSearch = document.all.xiaoxinarongSearch.value;
	ManageObj.dataStore.baseParams.jieshourenSearch = document.all.jieshourenSearch.value;
	...
	ManageObj.dataStore.baseParams.yueduzhuangtaiSearch = document.all.yueduzhuangtaiSearch.value;
...
/* 得到添加或编辑的FormPanel */
ManageObj.getOperationFormPan = function(rowIndex, flag ,jieshouren) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;消息标题',
					name : 'xiaoxibiaoti',
					id : 'xiaoxibiaoti',
					anchor : '95%',
					allowBlank : false
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;接收人',
					name : 'jieshouren',
					id : 'jieshouren',
					anchor : '95%',
					allowBlank : false
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;阅读状态',
					name : 'yueduzhuangtai',
					id : 'yueduzhuangtai',
					anchor : '95%',
					readOnly : true,
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('xiaoxibiaoti').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaoxibiaoti');
		ManageObj.operationFormPan.findById('xiaoxinarong').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaoxinarong');
		ManageObj.operationFormPan.findById('jieshouren').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('jieshouren');
		...
		ManageObj.operationFormPan.findById('yueduzhuangtai').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('yueduzhuangtai');
	...
	if( flag == "back" ){
		ManageObj.operationFormPan.findById('xiaoxibiaoti').value = "Re：" + ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaoxibiaoti');
		ManageObj.operationFormPan.findById('xiaoxinarong').value = "Re：" + ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaoxinarong');
		ManageObj.operationFormPan.findById('jieshouren').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('faburen');
		ManageObj.operationFormPan.findById('faburen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('jieshouren');
		ManageObj.operationFormPan.findById('yueduzhuangtai').value = "未读";
	}
	
	if( LoginUserAcct == jieshouren ){
		Ext.Ajax.request({
			waitMsg : '数据提交中......',
			url : '/WEB-ROOT/app/xiaoxiguanli/zhannaxiaoxi.do',
			method : 'POST',
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xiaoxiguanli/zhannaxiaoxi.do?flag=doExportExcel";
	window.open(url);
};
