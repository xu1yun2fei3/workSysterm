var ManageObj = function() {
	...
	ManageObj.kaopingmingchenGuanLianStore = new Ext.data.JsonStore({
		url : '/WEB-ROOT/app/wodekaohepingding/woyaopingding.do',
		baseParams : {
			flag : 'doGuanLian',
	...
	ManageObj.kaopingduixiangGuanLianStore = new Ext.data.JsonStore({
		url : '/WEB-ROOT/app/wodekaohepingding/woyaopingding.do',
		baseParams : {
			flag : 'doGuanLian',
			guanLianBiao : 'yuangongxinxi',
			guanLianZiDuan : 'xingming'
		},
		root : 'root',
		totalProperty : 'totalProperty',
		fields : [ 'xingming' ]
					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '考评名称',
							name : 'kaopingmingchenSearch',
							id : 'kaopingmingchenSearch',
							anchor : '100%'
						} ]
				}
				,
					...
					items : [
						{
							xtype : 'textfield',
							fieldLabel : '考评对象',
							name : 'kaopingduixiangSearch',
							id : 'kaopingduixiangSearch',
							anchor : '100%'
						} ]
				}
				,
					...
					items : [
						{
							xtype : 'datefield',
							fieldLabel : '提交时间',
							name : 'tijiaoshijianSearch',
							id : 'tijiaoshijianSearch',
							anchor : '100%'
						} ]
				}
				,
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		totalProperty : 'totalProperty',
		remoteSort : true,
		fields : [ 'kaopingmingchen', 'kaopingduixiang', 'tijiaoren', 'tijiaoshijian', 'shenpi', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd', 'attr1', 'attr2', 'attr3', 'attr4', 'attr5' ],
	...
	*/
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		columns : [
			...
			*/
			{
				header : '<font color="blue">考评名称</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'kaopingmingchen'
			},
			{
				header : '<font color="blue">考评对象</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'kaopingduixiang'
			},
			{
				header : '<font color="blue">提交人</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'tijiaoren'
			...
			{
				header : '<font color="blue">提交时间</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'tijiaoshijian'
			},
			{
				header : '<font color="blue">状态</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'shenpi'
...
ManageObj.operatorRenderer = function(id, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if (R == "n") {
		...
		*/
		...
		returnStr += "&nbsp;|&nbsp;<a href=# onclick=ManageObj.update4ej('/WEB-ROOT/app/wodekaohepingding/pingdingmingxi.do?r=y&erjiguanlianzd=" + record.data['id'] + "&m=pingdingmingxi','update');打分浏览</a>";
		returnStr += "&nbsp;|&nbsp;<a href=# onclick=ManageObj.update4ej('/WEB-ROOT/app/wodekaohepingding/pingdingmingxi.do?erjiguanlianzd=" + record.data['id'] + "&m=pingdingmingxi','update');>我要打分</a>";

	} else {
		returnStr += "&nbsp;|&nbsp;<a href=# onclick=ManageObj.update4ej('/WEB-ROOT/app/wodekaohepingding/pingdingmingxi.do?r=y&erjiguanlianzd=" + record.data['id'] + "&m=pingdingmingxi','update');>明细浏览</a>";
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.kaopingmingchenSearch = document.all.kaopingmingchenSearch.value;
	ManageObj.dataStore.baseParams.kaopingduixiangSearch = document.all.kaopingduixiangSearch.value;
	ManageObj.dataStore.baseParams.tijiaorenSearch = document.all.tijiaorenSearch.value;
	ManageObj.dataStore.baseParams.tijiaoshijianSearch = document.all.tijiaoshijianSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'combo',
					fieldLabel : '<font color="red">*</font>&nbsp;考评名称',
					store : ManageObj.kaopingmingchenGuanLianStore,
					valueField : 'kaopingmingchen',
					displayField : 'kaopingmingchen',
					mode : 'remote',
					forceSelection : false,
					hiddenName : 'kaopingmingchenHidd',
					triggerAction : 'all',
					editable : true,
					name : 'kaopingmingchen',
					id : 'kaopingmingchen',
					anchor : '95%',
					allowBlank : false
				} ]
				...
				, {
					...
					items : [ {
						xtype : 'combo',
						fieldLabel : '<font color="red">*</font>&nbsp;考评对象',
						store : ManageObj.kaopingduixiangGuanLianStore,
						valueField : 'xingming',
						displayField : 'xingming',
						mode : 'remote',
						forceSelection : false,
						hiddenName : 'kaopingduixiangHidd',
						triggerAction : 'all',
						editable : true,
						name : 'kaopingduixiang',
						id : 'kaopingduixiang',
						anchor : '95%',
						allowBlank : false
					} ]
				...
				, {
					...
					items : [ {
						xtype : 'textfield',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;提交人',
						name : 'tijiaoren',
						id : 'tijiaoren',
						anchor : '95%',
						readOnly : true,
				...
				, {
					...
					items : [ {
						xtype : 'datefield',
						format : 'Y-m-d',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;提交时间',
						name : 'tijiaoshijian',
						id : 'tijiaoshijian',
						anchor : '95%',
						readOnly : true,
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('kaopingmingchen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('kaopingmingchen');
		ManageObj.operationFormPan.findById('kaopingduixiang').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('kaopingduixiang');
		ManageObj.operationFormPan.findById('tijiaoren').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('tijiaoren');
		ManageObj.operationFormPan.findById('tijiaoshijian').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('tijiaoshijian');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/wodekaohepingding/woyaopingding.do?flag=doExportExcel";
	window.open(url);
};
