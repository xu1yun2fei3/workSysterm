...
{xtype : 'textfield',
fieldLabel : '岗位名称',
name : 'gangweimingchenSearch',
id : 'gangweimingchenSearch',
anchor : '100%'
} ]}
,
{
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore( {
		...
		remoteSort : true,
		fields : [ 'gangweimingchen','id','itime','detail','deleteFlag','erjiguanlianzd','attr1','attr2','attr3','attr4','attr5' ],
		proxy : new Ext.data.HttpProxy( {
			url : '/WEB-ROOT/app/xitonggaojishezhi/gangweishezhi.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	*/
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel( {
		...
		*/
		{
header : '<font color="blue">岗位名称</font>',
width : 60,
align : 'center',
sortable : true,
dataIndex : 'gangweimingchen'
}, 
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.gangweimingchenSearch = document.all.gangweimingchenSearch.value; 
...
,{
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('gangweimingchen').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('gangweimingchen');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xitonggaojishezhi/gangweishezhi.do?flag=doExportExcel";
	window.open(  url );
};
