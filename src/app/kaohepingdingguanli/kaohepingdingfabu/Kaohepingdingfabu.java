package  app.kaohepingdingguanli.kaohepingdingfabu;
...
public class Kaohepingdingfabu extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/kaohepingdingguanli/kaohepingdingfabu/kaohepingdingfabu.jsp", resultMap, request,  response);
            }
            //提交数据库
...
String kaopingmiaoshuSearch = request.getParameter("kaopingmiaoshuSearch"); 
 if( kaopingmiaoshuSearch != null && !kaopingmiaoshuSearch.equals("") ){  
 	 where.put("kaopingmiaoshuSearch", kaopingmiaoshuSearch);  
 } 
String canyurenSearch = request.getParameter("canyurenSearch"); 
 if( canyurenSearch != null && !canyurenSearch.equals("") ){  
 	 where.put("canyurenSearch", canyurenSearch);  
 } 
String tijiaoriqiSearch = request.getParameter("tijiaoriqiSearch"); 
 if( tijiaoriqiSearch != null && !tijiaoriqiSearch.equals("") ){  
 	 where.put("tijiaoriqiSearch", tijiaoriqiSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Kaohepingdingfabu.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Kaohepingdingfabu.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("kaopingmingchen", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("kaopingmiaoshu", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("canyuren", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("tijiaoriqi", item.getString("gb2312"));
}
else if (i == 4) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Kaohepingdingfabu.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Kaohepingdingfabu.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Kaohepingdingfabu.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Kaohepingdingfabu.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Kaohepingdingfabu.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Kaohepingdingfabu.selecteList", where);
 ...
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "考评描述", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "参与人", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "提交日期", wcfFC5);  
 ws.addCell(label_3); 
 ...
 ws.addCell(label_0); 
 String kaopingmiaoshu = ((HashMap) (al.get(i))).get("kaopingmiaoshu").toString();  
 label_1 = new Label(2, i + 4, kaopingmiaoshu, wcfF4); 
 ws.addCell(label_1); 
 String canyuren = ((HashMap) (al.get(i))).get("canyuren").toString();  
 label_2 = new Label(3, i + 4, canyuren, wcfF4); 
 ws.addCell(label_2); 
 String tijiaoriqi = ((HashMap) (al.get(i))).get("tijiaoriqi").toString();  
 label_3 = new Label(4, i + 4, tijiaoriqi, wcfF4); 
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Kaohepingdingfabu.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Kaohepingdingfabu.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Kaohepingdingfabu.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Kaohepingdingfabu.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Kaohepingdingfabu.doDelePic", where);
