package  app.bumenxinxiguanli.bumenxinxi;
...
public class Bumenxinxi extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/bumenxinxiguanli/bumenxinxi/bumenxinxi.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String bumenmingchenSearch = request.getParameter("bumenmingchenSearch"); 
 if( bumenmingchenSearch != null && !bumenmingchenSearch.equals("") ){  
 	 where.put("bumenmingchenSearch", bumenmingchenSearch);  
 } 
String shangjibumenSearch = request.getParameter("shangjibumenSearch"); 
 if( shangjibumenSearch != null && !shangjibumenSearch.equals("") ){  
 	 where.put("shangjibumenSearch", shangjibumenSearch);  
 } 
String bumenzhizeSearch = request.getParameter("bumenzhizeSearch"); 
 if( bumenzhizeSearch != null && !bumenzhizeSearch.equals("") ){  
 	 where.put("bumenzhizeSearch", bumenzhizeSearch);  
 } 
String chenglishijianSearch = request.getParameter("chenglishijianSearch"); 
 if( chenglishijianSearch != null && !chenglishijianSearch.equals("") ){  
 	 where.put("chenglishijianSearch", chenglishijianSearch);  
 } 
String bumenfuzerenSearch = request.getParameter("bumenfuzerenSearch"); 
 if( bumenfuzerenSearch != null && !bumenfuzerenSearch.equals("") ){  
 	 where.put("bumenfuzerenSearch", bumenfuzerenSearch);  
 } 
String bianzhirenshuSearch = request.getParameter("bianzhirenshuSearch"); 
 if( bianzhirenshuSearch != null && !bianzhirenshuSearch.equals("") ){  
 	 where.put("bianzhirenshuSearch", bianzhirenshuSearch);  
 } 
String bumenmiaoshuSearch = request.getParameter("bumenmiaoshuSearch"); 
 if( bumenmiaoshuSearch != null && !bumenmiaoshuSearch.equals("") ){  
 	 where.put("bumenmiaoshuSearch", bumenmiaoshuSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Bumenxinxi.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String bumenmingchen = request.getParameter("bumenmingchen"); 
where.put("bumenmingchen", bumenmingchen);  
String shangjibumen = request.getParameter("shangjibumen"); 
where.put("shangjibumen", shangjibumen);  
String bumenzhize = request.getParameter("bumenzhize"); 
where.put("bumenzhize", bumenzhize);  
String chenglishijian = request.getParameter("chenglishijian"); 
where.put("chenglishijian", chenglishijian);  
String bumenfuzeren = request.getParameter("bumenfuzeren"); 
where.put("bumenfuzeren", bumenfuzeren);  
String bianzhirenshu = request.getParameter("bianzhirenshu"); 
where.put("bianzhirenshu", bianzhirenshu);  
String bumenmiaoshu = request.getParameter("bumenmiaoshu"); 
where.put("bumenmiaoshu", bumenmiaoshu);  
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库新增数据
            sqlMap.insert("Bumenxinxi.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
            ...
            //新增数据
		    sqlMap.insert("Bumenxinxi.insertObj", whereMap);
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库提交修改数据
            sqlMap.update("Bumenxinxi.updateObj", where);
    ...
    public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//向数据库提交修改信息
		    sqlMap.update("Bumenxinxi.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Bumenxinxi.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Bumenxinxi.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Bumenxinxi.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "部门名称", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "上级部门", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "部门职责", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "成立时间", wcfFC5);  
 ws.addCell(label_3); 
 Label label_4 = new Label(5, 3, "部门负责人", wcfFC5);  
 ws.addCell(label_4); 
 Label label_5 = new Label(6, 3, "编制人数", wcfFC5);  
 ws.addCell(label_5); 
 Label label_6 = new Label(7, 3, "部门描述", wcfFC5);  
 ws.addCell(label_6); 
 ...
 ws.addCell(label_9); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String bumenmingchen = ((HashMap) (al.get(i))).get("bumenmingchen").toString();  
 label_0 = new Label(1, i + 4, bumenmingchen, wcfF4); 
 ws.addCell(label_0); 
 String shangjibumen = ((HashMap) (al.get(i))).get("shangjibumen").toString();  
 label_1 = new Label(2, i + 4, shangjibumen, wcfF4); 
 ws.addCell(label_1); 
 String bumenzhize = ((HashMap) (al.get(i))).get("bumenzhize").toString();  
 label_2 = new Label(3, i + 4, bumenzhize, wcfF4); 
 ws.addCell(label_2); 
 String chenglishijian = ((HashMap) (al.get(i))).get("chenglishijian").toString();  
 label_3 = new Label(4, i + 4, chenglishijian, wcfF4); 
 ws.addCell(label_3); 
 String bumenfuzeren = ((HashMap) (al.get(i))).get("bumenfuzeren").toString();  
 label_4 = new Label(5, i + 4, bumenfuzeren, wcfF4); 
 ws.addCell(label_4); 
 String bianzhirenshu = ((HashMap) (al.get(i))).get("bianzhirenshu").toString();  
 label_5 = new Label(6, i + 4, bianzhirenshu, wcfF4); 
 ws.addCell(label_5); 
 String bumenmiaoshu = ((HashMap) (al.get(i))).get("bumenmiaoshu").toString();  
 label_6 = new Label(7, i + 4, bumenmiaoshu, wcfF4); 
 ws.addCell(label_6); 
 ...
 ws.addCell(label_9); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Bumenxinxi.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Bumenxinxi.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Bumenxinxi.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Bumenxinxi.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Bumenxinxi.doDelePic", where);
