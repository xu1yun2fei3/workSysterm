 ...
 *            rowIndex: 编辑行的行号
 * @param {}
 *            flag：操作标识（'update'为编辑；'view'为浏览）
 */
ManageObj.update = function(rowIndex, flag, jieshouren) {
	...
	ManageObj.updateWindow = new Ext.Window({
		...
		resizable : false,
		layout : 'fit',
		items : ManageObj.getOperationFormPan(rowIndex, flag, jieshouren)
	});
	...
	}else if (flag != null && flag == "back") {
		ManageObj.updateWindow.addButton("回复", function() {
			ManageObj.doAddSubmit("back");
		});
	}
...
ManageObj.doUpdateSubmit = function(rowIndex) {
	...
	ManageObj.operationFormPan.getForm().submit({
		waitMsg : '数据提交中......',
		method : 'post',
		url : '/WEB-ROOT/app/xiaoxiguanli/zhannaxiaoxi.do?flag=doUpdateSubmit' + '&id=' + ManageObj.dataGridPan.getStore().getAt(rowIndex).get('id') + "&erjiguanlianzd=" + erjiguanlianzd + '&isChangeAttach=' + isChangeAttach,
