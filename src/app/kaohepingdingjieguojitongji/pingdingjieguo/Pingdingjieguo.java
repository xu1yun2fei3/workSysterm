package  app.kaohepingdingjieguojitongji.pingdingjieguo;
...
public class Pingdingjieguo extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/kaohepingdingjieguojitongji/pingdingjieguo/pingdingjieguo.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String kaopingxiangmuSearch = request.getParameter("kaopingxiangmuSearch"); 
 if( kaopingxiangmuSearch != null && !kaopingxiangmuSearch.equals("") ){  
 	 where.put("kaopingxiangmuSearch", kaopingxiangmuSearch);  
 } 
String beikaopingrenSearch = request.getParameter("beikaopingrenSearch"); 
 if( beikaopingrenSearch != null && !beikaopingrenSearch.equals("") ){  
 	 where.put("beikaopingrenSearch", beikaopingrenSearch);  
 } 
String kaopingfenshuSearch = request.getParameter("kaopingfenshuSearch"); 
 if( kaopingfenshuSearch != null && !kaopingfenshuSearch.equals("") ){  
 	 where.put("kaopingfenshuSearch", kaopingfenshuSearch);  
 } 
...
String xiangxishuomingSearch = request.getParameter("xiangxishuomingSearch"); 
 if( xiangxishuomingSearch != null && !xiangxishuomingSearch.equals("") ){  
 	 where.put("xiangxishuomingSearch", xiangxishuomingSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingjieguo.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Pingdingjieguo.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("kaopingxiangmu", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("beikaopingren", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("kaopingfenshu", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("pingdingdengji", item.getString("gb2312"));
}
else if (i == 4) {
	whereMap.put("xiangxishuoming", item.getString("gb2312"));
}
else if (i == 5) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Pingdingjieguo.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Pingdingjieguo.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Pingdingjieguo.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Pingdingjieguo.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Pingdingjieguo.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Pingdingjieguo.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "考评项目", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "被考评人", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "考评分数", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "评定等级", wcfFC5);  
 ws.addCell(label_3); 
 Label label_4 = new Label(5, 3, "详细说明", wcfFC5);  
 ws.addCell(label_4); 
 ...
 ws.addCell(label_7); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String kaopingxiangmu = ((HashMap) (al.get(i))).get("kaopingxiangmu").toString();  
 label_0 = new Label(1, i + 4, kaopingxiangmu, wcfF4); 
 ws.addCell(label_0); 
 String beikaopingren = ((HashMap) (al.get(i))).get("beikaopingren").toString();  
 label_1 = new Label(2, i + 4, beikaopingren, wcfF4); 
 ws.addCell(label_1); 
 String kaopingfenshu = ((HashMap) (al.get(i))).get("kaopingfenshu").toString();  
 label_2 = new Label(3, i + 4, kaopingfenshu, wcfF4); 
 ws.addCell(label_2); 
 String pingdingdengji = ((HashMap) (al.get(i))).get("pingdingdengji").toString();  
 label_3 = new Label(4, i + 4, pingdingdengji, wcfF4); 
 ws.addCell(label_3); 
 String xiangxishuoming = ((HashMap) (al.get(i))).get("xiangxishuoming").toString();  
 label_4 = new Label(5, i + 4, xiangxishuoming, wcfF4); 
 ws.addCell(label_4); 
 ...
 ws.addCell(label_7); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Pingdingjieguo.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Pingdingjieguo.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Pingdingjieguo.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingjieguo.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Pingdingjieguo.doDelePic", where);
