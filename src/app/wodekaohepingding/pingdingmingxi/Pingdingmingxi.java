package  app.wodekaohepingding.pingdingmingxi;
...
public class Pingdingmingxi extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/wodekaohepingding/pingdingmingxi/pingdingmingxi.jsp", resultMap, request,  response);
            }
            //提交数据库
...
String defenshuomingSearch = request.getParameter("defenshuomingSearch"); 
 if( defenshuomingSearch != null && !defenshuomingSearch.equals("") ){  
 	 where.put("defenshuomingSearch", defenshuomingSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingmingxi.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String kaopingxiangmu = request.getParameter("kaopingxiangmuHidd"); 
where.put("kaopingxiangmu", kaopingxiangmu);  
String kaopingfenshu = request.getParameter("kaopingfenshuHidd"); 
where.put("kaopingfenshu", kaopingfenshu);  
String defenshuoming = request.getParameter("defenshuoming"); 
where.put("defenshuoming", defenshuoming);  
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库新增数据
            sqlMap.insert("Pingdingmingxi.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
            ...
            //新增数据
		    sqlMap.insert("Pingdingmingxi.insertObj", whereMap);
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库提交修改数据
            sqlMap.update("Pingdingmingxi.updateObj", where);
    ...
    public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//向数据库提交修改信息
		    sqlMap.update("Pingdingmingxi.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.update("Pingdingmingxi.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.update("Pingdingmingxi.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Pingdingmingxi.selecteList", where);
 ...
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "考评分数", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "得分说明", wcfFC5);  
 ws.addCell(label_2); 
 ...
 label_1 = new Label(2, i + 4, kaopingfenshu, wcfF4); 
 ws.addCell(label_1); 
 String defenshuoming = ((HashMap) (al.get(i))).get("defenshuoming").toString();  
 label_2 = new Label(3, i + 4, defenshuoming, wcfF4); 
 ws.addCell(label_2); 
 ...
 ws.addCell(label_5); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Pingdingmingxi.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Pingdingmingxi.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Pingdingmingxi.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Pingdingmingxi.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Pingdingmingxi.doDelePic", where);
